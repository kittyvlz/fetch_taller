  ...
  function showData(dataArray) {
    container.innerHTML= '';// Limpia el contenido del contenedor antes de agregar los elementos
    ...
    for (const item of dataArray) {
      ...
      container.innerHTML += `<div> ${item.name} ${item.lastname} </div>`; // Se concatena cada párrafo de la manera que queremos mostrarlo al innerHTML del contenedor
    }
  }
  ...
  fetch(DATA_URL)
    ...
    .then (({course, students, teacherName}) => {
      document.getElementById('title').innerHTML = ` Muestra a los Estudiantes del curso de <span>${course}</span>`  // Actualizo el título con el nombre del curso
